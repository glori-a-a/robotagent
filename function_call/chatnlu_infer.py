...
# If BERT has no stop class, still expose Stop_Movement to the LLM and map it for the API response.
...
def predict(query, trace_id):
    try:
        ...
        logger.info(f"LLM tool_calls: {result}")
        logger.info(f"LLM call time: {time.time() - start_time}")
        if not result:
            return "未知-无"
...
async def inference(request: Request):
    ...
    # Intent string and slots from predict()
    nlu = predict(query, trace_id)


    # Parse intent-id and slot dict for JSON response
    nlu_items = nlu.split("-")
    intent = nlu_items[0]
    if len(nlu_items) > 2:
